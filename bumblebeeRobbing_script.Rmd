...
eps <- 0.01
m_rob <- lmer (log(timetorob+eps) ~ treatment + (1| sample) + (1| plantsp) + (1| flower.) + (1| species), data = robbed_flowers)
summary(m_rob)
...
m_simpler <- lmer(log(timetorob + eps) ~ treatment + 
                  (1|sample) + (1|flower.),
                  data = robbed_flowers)

anova(m_simpler, m_rob)  # compare fits with likelihood ratio test

m_final <- lmer(log(timetorob + eps) ~ treatment + plantsp + species +
                (1|sample) + (1|flower.),
                data = robbed_flowers)
...
obs <- robbed_flowers %>% distinct(plantsp, species)
...
robbed_flowers <- robbed_flowers %>%
  mutate(group = interaction(species, plantsp, sep = " × "))

# 2) Get emmeans by treatment within plant × bee, back to seconds, keep only observed combos
emm_full <- emmeans(m_full, ~ treatment | plantsp * species)
emm_df <- as.data.frame(regrid(emm_full, transform = "response"))
...
timetorob_emm <- ggplot(robbed_flowers,
...
```{r}
m_feed <- lmer (log(timefeeding+eps) ~ treatment + (1| sample) + (1| plantsp) + (1| flower.) + (1| species), data = robbed_flowers)
summary(m_feed)
...
emm_df_obs_feed <- emm_df_feed %>% semi_join(obs_feed, by = c("plantsp","species"))%>% mutate(group = interaction(species, plantsp, sep = " × "))
